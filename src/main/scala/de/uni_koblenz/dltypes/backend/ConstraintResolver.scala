...
class ConstraintResolver(val reasoner: Reasoner) {
  ...
  case class RoleConstraint(x: Variable, r: Role, n: Nominal) extends CS {
    def get =
      if (Globals.doABoxReasoning)
        Map(x -> Existential(r, n))
      else
        Map(x -> Existential(r, Top))
  }
  ...
  case class InvRoleConstraint(x: Variable, i: Inverse, n: Nominal) extends CS {
    def get =
      if (Globals.doABoxReasoning)
        Map(x -> Existential(i, n))
      else
        Map(x -> Existential(i, Top))
  }
  ...
  private def qexprToCS(qexpr: QueryExpression): Option[CS] = {
    ...
    case RoleAssertion(Var(x1), Var(x2), Iri(r)) =>
      if (false) //(reasoner.isData(r))
        //Some(Role2Constraint(Variable(x1), Variable(x2), Data(r)))
        Some(Role2Constraint(Variable(x1), Variable(x2), Role(r)))
      else
        Some(Role2Constraint(Variable(x1), Variable(x2), Role(r)))
  ...
  def eval(vs: Seq[Var],
    ...
    qexprToCS(qe) match {
      ...
      case Some(cs) => {
        ...
        val res =
          if (strict || Globals.strictQueryTyping) {
